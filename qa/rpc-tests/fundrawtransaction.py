...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        inputs  = [ {'txid' : utx['txid'], 'vout' : utx['vout']}]
        # Prux: Fee is exact, do not use tolerance
        ...
        assert_raises_jsonrpc(-5, "changeAddress must be a valid prux address", self.nodes[2].fundrawtransaction, rawtx, {'changeAddress':'foobar'})
        ...
        inputs  = [ {'txid' : utx['txid'], 'vout' : utx['vout']} ]
        # Prux: Reduce this output so the fee doesn't leave us with no change
        ...
        inputs = []
        # Prux: TX size rounding gives us a fee of 4 PRUX
