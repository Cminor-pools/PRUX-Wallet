...
Prux Core 1.8 introduces AuxPoW from block 371,337. AuxPoW is a technology
which enables miners to submit work done while mining other coins, as work
on the Prux block chain. Prux Core 1.8 also enables payment protocol
support for Prux (note that Prux's implementation of payment protocol is
...
as soon as possible afterwards in order to be able to continue to use Prux.
Users remaining on the previous client **will not** be able to receive or spend
Prux (with the exception of with other nodes who have not upgraded).
...
use "pruxd" you must run it the first time with the "-reindex" command line
option.
...
AuxPoW benefits Prux in two ways; firstly, the effective hashrate of the coin is
...
multipools from switching between Prux and other Scrypt coins.
...
In comparison to the AuxPoW implementation in United Scrypt Coin, Prux uses a
different coinbase transaction input script format which includes block height in
compliance with BIP0034.

## Payment Protocol

Prux Core 1.8 adds payment protocol support, which is used to streamline payments
being made to merchants.

This protocol is defined in DIP0070-DIP0072, based on the BIP standards with the same
assigned numbers. DIP standards can be found at https://github.com/prux/dips/ .
The key differencs between the Bitcoin and Prux payment protocols are that
the Prux payment request uses a "genesis" field containing the hash of the network's
...
(typically 1 Prux, or around $0.00012 per transaction).
...
of Prux compared to Bitcoin. This should lead to less false positives.

A problem in parsing mangled prux: URIs under Windows has been resolved, and as
part of this work network detection for prux: URIs is now more robust.
