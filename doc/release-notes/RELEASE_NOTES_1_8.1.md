...
Prux Core 1.8.1 is primarily a bugfix release, bringing Prux Core in
line with Bitcoin 0.9.3. Prux Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Prux Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Prux Core clients
* Use HTTPS with pruxchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/BITRY/PRUX-Wallet/issues/134).
...
This only affects values significantly below 1 Prux, however merchants may wish to round to
the nearest Prux on all transactions for now.
